#!/usr/bin/env python3
"""
Batch-download Instagram reels from a CSV using yt-dlp.
...
Examples:
  python download_instagram_reels.py \
      --csv "/path/to/cleaned_instagram_reels.csv" \
      --output-dir "%USERPROFILE%/Downloads/instagram"
...
DEFAULT_OUTPUT_DIR = Path.home() / "Downloads" / "instagram"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Download Instagram reels from a CSV using yt-dlp")
    parser.add_argument(
        "--csv",
        dest="csv_path",
        default=str(Path(__file__).with_name("cleaned_instagram_reels.csv")),
        help="Path to the cleaned CSV file",
    )
    parser.add_argument(
        "--output-dir",
        default=str(DEFAULT_OUTPUT_DIR),
        help="Folder where videos should be saved",
    )
    parser.add_argument(
        "--yt-dlp-path",
        default="yt-dlp",
    ...
    if not args.dry_run and shutil.which(args.yt_dlp_path) is None and not Path(args.yt_dlp_path).exists():
        print(
            "ERROR: yt-dlp was not found. Install it or pass --yt-dlp-path with the full path to yt-dlp.exe",
            file=sys.stderr,
        )
        return 1
